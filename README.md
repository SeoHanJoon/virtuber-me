...
3. **멀티 룸 멀티플레이어** - `/room/:id` 형태의 독립 룸 시스템 🆕
4. **참여자 목록 UI** - 실시간 참여자 표시 및 상태 확인 🆕
5. **VRM 수동 제어** - 슬라이더로 본 회전 직접 조절
6. **BlendShapes 모니터** - 실시간 표정 파라미터 시각화
7. **랜드마크 시각화** - 얼굴/상체 keypoints 오버레이
...
| --------------------- | ----------------------------------- |
| `/`                   | 메인 페이지 (룸 생성/입장)          |
| `/room/:id`           | 멀티플레이어 룸 (페이셜 트래킹) 🆕  |
| `/vrm`                | VRM 뷰어                            |
| `/face-tracking`      | 얼굴 추적 + VRM (단일 사용자)       |
| `/body-tracking-test` | 상체 추적 테스트 (실험적)           |
| `/multiplayer`        | 멀티플레이어 월드 (단일 룸, 레거시) |
...
3. **친구 초대**
   ...
   ```
   URL 공유: http://localhost:3000/room/ABC123
   → 친구가 같은 룸 ID로 입장
   ```
...
- [x] ✅ 1단계. 얼굴 트래킹 완성
- [x] ✅ 2단계. 멀티 룸 시스템 구현
- [x] ✅ 3단계. 참여자 UI 및 실시간 동기화
- ~~[ ] 상체 트래킹 안정화~~ (실패 - 단일 카메라 깊이감 한계)
- [ ] 🚀 음성 채팅 (WebRTC)
- [ ] 🎨 커스텀 아바타 장식 및 이펙트
- [ ] 📊 룸 통계 및 관리 대시보드
- [ ] ☁️ 프로덕션 배포 (Vercel + Railway)
